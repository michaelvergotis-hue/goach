...
export default function DashboardPage() {
  ...
  // Fetch posts when group changes or tab is feed
  useEffect(() => {
      ...
      <div className="px-5 max-w-6xl mx-auto w-full">
        <div className="flex items-center gap-1 p-1 bg-card rounded-xl">
          ...
          {friend.isAdmin && (
            <Link
              href="/admin"
              className="flex-1 py-2.5 px-4 rounded-lg font-medium text-sm text-muted hover:text-foreground text-center transition-all"
            >
              Admin
            </Link>
          )}
